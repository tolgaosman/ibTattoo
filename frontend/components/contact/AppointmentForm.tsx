...
export function AppointmentForm() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    const datesStr = values.dates?.from
      ? `\nTarih Aralığı: ${values.dates.from.toLocaleDateString("tr-TR")}${values.dates.to ? ` - ${values.dates.to.toLocaleDateString("tr-TR")}` : ""}`
      : "";

    const message = `Merhaba, dövme randevusu için yazıyorum.\n\nAd Soyad: ${values.name}\nBölge: ${values.placement}\nBoyut: ${values.size} cm\n\nFikir: ${values.idea}${datesStr}`;

    // WhatsApp'ın wa.me linki dosya eki taşıyamıyor; görseli mesajla birlikte
    // gerçekten göndermek için mümkün olduğunda cihazın paylaşım menüsünü
    // (Web Share API) kullanıyoruz, desteklenmeyen tarayıcılarda metne düşüyoruz.
    if (referenceFile && typeof navigator !== "undefined" && navigator.canShare?.({ files: [referenceFile] })) {
      try {
        await navigator.share({ text: message, files: [referenceFile] });
        return;
      } catch {
        // Kullanıcı paylaşımı iptal etti veya paylaşım başarısız oldu; wa.me'ye düşüyoruz.
      }
    }

    const fallbackMessage = referenceFile
      ? `${message}\n\n(Referans görselimi ayrıca buradan göndereceğim.)`
      : message;

    window.open(`https://wa.me/905338668490?text=${encodeURIComponent(fallbackMessage)}`, "_blank");
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
      ...
      <Field label="Referans Görsel Yükle" hint="WhatsApp'a görsel olarak gönderilecek (Opsiyonel)">
        <input
          type="file"
          accept="image/*"
          onChange={(event) => setReferenceFile(event.target.files?.[0] ?? null)}
          className="w-full text-sm text-ink-soft file:mr-4 file:rounded-[var(--radius-sm)] file:border-0 file:bg-sand/30 file:px-4 file:py-2.5 file:text-sm file:text-amber file:transition-colors file:duration-200 hover:file:bg-sand/50"
        />
      </Field>
